...
<?php
		...
		if(isset($_POST['UpdateButton'])){
			$u_publisherID     = $_POST['UpdateButton'];
			$u_pName	       = $_POST['U_pName'];
	
			$u_sql = mysql_query("UPDATE Publishers SET pName='".$u_pName."' WHERE publisherID=$u_publisherID");
			//echo $usql;
		...
		<table style=" border:1px solid black; border-collapse: collapse; text-align: left;background-color: #F5F5F5;">
		  ...
		  <?php
            ...
            while ($row = mysql_fetch_assoc($result)) {
                echo
                "<form action='publishers.php' method='POST'><tr>
					<td>{$row['publisherID']}</td>
					<td><input type='text' name='U_pName' value='" . $row['pName'] . "'</td>";
				echo '<td><button class="update-button" name="UpdateButton'
				.'" value="' . $row['publisherID'] . '"/>Update</td></button>';
				echo '<td></form></td>';
				echo "</tr>";
			}
            ?>
